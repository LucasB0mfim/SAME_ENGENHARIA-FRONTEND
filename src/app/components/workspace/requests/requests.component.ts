...
import { IOrderRequest } from '../../../core/interfaces/order-request.interface';
...
export class RequestsComponent implements OnInit {
  ...
  find() {
    this._requestService.find().subscribe({
      next: (data) => {
        this.records = data.order.filter(data => data.status === null);
        this.allRecords = data.order.filter(data => data.status === null);
      },
      error: (error) => {
        console.error(error);
      }
  ...
  NotDelivered(index: number) {
    ...
    formData.append('oc', currentRecord.oc.toString()); // Adiciona o ID do pedido
